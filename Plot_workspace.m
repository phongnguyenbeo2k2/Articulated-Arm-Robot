function Plot_workspace(x_denta,z_denta)
%% brief: x and z is position of joint 2
...
x = r1 * cos(theta) + x_denta;
z = r1 * sin(theta) + z_denta;
rotate_and_draw(x, z);

% TH2: R2
r2 = sqrt(a2^2 + a3^2 - 2*a2*a3*cos(pi/9));
theta2 = linspace(-pi/2, pi/2, 100);
x = r2 * cos(theta2) + x_denta;
z = r2 * sin(theta2) + z_denta;
rotate_and_draw(x, z);
...
x = r3 * cos(theta3) + x_denta;
z = r3 * sin(theta3) + z_denta;
rotate_and_draw(x, z);
...
x = r4 * cos(theta4) + denta_x1 + x_denta;
z = r4 * sin(theta4) + denta_z1 + z_denta;
rotate_and_draw(x, z);
...
x = r5 * cos(theta5) + denta_x2 + x_denta;
z5 = r5 * sin(theta5) + denta_z2 + z_denta;
rotate_and_draw(x, z5);

%line ben tren
% Tạo đường thẳng vuông góc với mặt phẳng Oxy
x = linspace(0 + x_denta, 0 + x_denta, 100); % Giá trị x cố định tại 0
y = linspace(0, 0, 100); % Giá trị y cố định tại 0
z = linspace(r2 + z_denta, r3 + z_denta, 100); % Giá trị z thay đổi từ r2 đến r3
...
x = linspace(0 + x_denta, 0 + x_denta, 100); % Giá trị x cố định tại 0
y = linspace(0, 0, 100); % Giá trị y cố định tại 0
z = linspace(-r2 + z_denta, z_th5 + z_denta, 100); % Giá trị z thay đổi từ r2 đến r3
% Vẽ đường thẳng bằng cách quay xung quanh trục z
plot3(x,y,z,'LineWidth',1,'Color','b');
hold off;
...
function draw_surface(x, z, phi)
    ...
    surf(X, Y, Z, 'FaceAlpha', 0.3, 'EdgeColor', 'blue');
end
